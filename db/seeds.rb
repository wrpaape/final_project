...
def format_text(raw, ar_keywords)
  split_lines = raw.split("\n")
  formatted_lines = split_lines.map do |line|
    comment_segs = line.split("# ")
    formatted_segs = comment_segs.map.with_index do |comment_seg, i|
      if i.odd?
        comment_seg.prepend("▓@# ") << "▓"
      else
        ar_keywords.each do |ar_key|
          comment_seg.gsub!(/(?<![A-Za-z_0-9])#{ar_key}(?![A-Za-z_0-9])/, "▓##{ar_key}▓")
        end
...
ar_keywords = all_methods.map { |ar_method| ar_method[:name] }
all_methods.each.with_index do |ar_method, i|
  ar_method.each do |key, val|
    val ||= "nil"
    if [:description, :example, :source].include?(key) && val.length > 48
      all_methods[i][key] = format_text(val, ar_keywords)
    end
  end

  ActRecMethod.create(ar_method)
end
...
def answer_the_red_line
  red_line = Client.joins(:contracts).select("clients.*, revenue - SUM(price * weight) AS income").group(:id).order("income").take
  black_thumb = Farmer.joins(:contracts, :farm).select("farmers.*, SUM(price * weight) - maintenance - (SELECT SUM(upkeep) FROM fields WHERE farm_id = farms.id) AS income").group("farmers.id, farms.id").order("income").take
  [red_line.name, black_thumb.name]
end
...
def task_todo
  rand(1..15).times.map do |i|
    ...
    Faker::Hacker.say_something_smart].sample.prepend("#{i}: ")
  end
end
